...
- [ ] **PRIMEIRO: ajustar o cenário do Make.** O cenário ativo grava no
      Firestore usando o token do próprio cliente, e esse acesso passa pelas
      regras de segurança. Ele precisa passar a gravar com o módulo
      "Google Cloud Firestore" (conexão Google do dono do projeto), gravar só
      `email` + `plano` (sem `dataVencimento`) e tratar cliente que já existe
      (upgrade de Básico para PRO).

- [ ] **Testar uma compra real (ou de teste)** e conferir no Firestore se o
      documento do cliente foi criado com `plano` correto.

- [ ] **SÓ DEPOIS: publicar a regra do Firestore.** Copie o conteúdo de
      `firestore.rules` e cole em *Firebase Console → Firestore Database →
      Regras → Publicar*. Se publicar antes de ajustar o Make, novos
      compradores ficam sem acesso.
